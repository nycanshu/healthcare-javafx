...
/**
 * Test ResidentService implementation using H2 test database
 */
public class TestResidentService implements IResidentService {
    ...
    public Resident admitResident(Resident resident, Long bedId) {
        // Set the bed ID and admission date
        resident.setCurrentBedId(bedId);
        resident.setAdmissionDate(java.time.LocalDate.now());
    ...
    public void dischargeResident(Long residentId) {
        String sql = "UPDATE Residents SET discharge_date = ? WHERE resident_id = ?";
    ...
    public List<Resident> findActiveResidents() {
        String sql = "SELECT * FROM Residents WHERE discharge_date IS NULL ORDER BY resident_id";
        List<Resident> residentList = new ArrayList<>();
    ...
    public List<Resident> findDischargedResidents() {
        String sql = "SELECT * FROM Residents WHERE discharge_date IS NOT NULL ORDER BY resident_id";
        List<Resident> residentList = new ArrayList<>();
    ...
    /**
     * Map ResultSet to Resident object
     */
    private Resident mapResultSetToResident(ResultSet rs) throws SQLException {
        ...
        if (birthDate != null) {
            resident.setBirthDate(birthDate.toLocalDate());
        }
        
        Date admissionDate = rs.getDate("admission_date");
        if (admissionDate != null) {
            resident.setAdmissionDate(admissionDate.toLocalDate());
        }
        
        Date dischargeDate = rs.getDate("discharge_date");
        if (dischargeDate != null) {
            resident.setDischargeDate(dischargeDate.toLocalDate());
        }
        
        resident.setCurrentBedId(rs.getLong("current_bed_id"));
        resident.setAssignedDoctorId(rs.getLong("assigned_doctor_id"));
